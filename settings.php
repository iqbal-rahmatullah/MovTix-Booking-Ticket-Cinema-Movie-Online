...
if (isset($_POST['change'])) {
    ...
    $cek = hasil_query("SELECT * FROM user WHERE id = '$id_res'")[0];

    if (password_verify($password, $cek['password'])) {
        if ($new_password != $retype_password) {
            $status_pw = false;
            $pesan_pw = "The new password you entered is not the same";
        } else {
            $status_pw = true;
            $pesan_pw = "Your password has been successfully changed!";

            $new_password = password_hash($new_password, PASSWORD_DEFAULT);
            $query_pw = "UPDATE user SET password = '$new_password' WHERE id = $id_res";
            mysqli_query($koneksi, $query_pw);
        }
    } else {
        $status_pw = false;
        $pesan_pw = "The old password you entered is incorrect";
    }
}
?>
<!DOCTYPE html>
                ...
                <div class="content-wrapper">
                    <div class="row">
                        <div class="col-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <h3 class="card-title text-center"><i class="mdi mdi-settings"></i> Change Password</h3>
                                    <div class="alert <?php if ($status_pw) {
                                                            echo "alert-success";
                                                        } else echo "alert-danger"; ?> <?php if (!isset($status_pw)) echo "d-none" ?>" role="alert">
                                        <?php echo $pesan_pw ?>
                                    </div>
                                    <form class="forms-sample" method="POST" action="">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group-prepend">
                                                <span class="input-group-text bg-info"><i class="mdi mdi-account-key text-white"></i></span>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group-prepend">
                                                <span class="input-group-text bg-warning"><i class="mdi mdi-key text-white"></i></span>
                                                <input type="password" class="form-control" id="new_password" placeholder="Your New Password" name="new_password">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="input-group-prepend">
                                                <span class="input-group-text bg-warning"><i class="mdi mdi-key text-white"></i></span>
                                                <input type="password" class="form-control" id="retype_password" placeholder="Re-type your password" name="retype_password">
                                            </div>
                                        </div>

                                        <div class="d-flex justify-content-center">
                                            <button type="submit" name="change" class="btn btn-primary mr-2 px-4 py-3">Change</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
