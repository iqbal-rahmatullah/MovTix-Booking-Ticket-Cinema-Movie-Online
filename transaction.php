...
if (!isset($_POST['tanggal']) || !isset($_POST['total_tiket']) || !isset($_POST['id_movie']) || !isset($_POST['tanggal'])) {
...
$daftar_kursi_dipesan = array();

for ($i = 1; $i <= $total_tiket; $i++) {
    ${"nama" . $i} = $_POST['nama' . $i];
    ${"umur" . $i} = $_POST['umur' . $i];
    ${"no_kursi" . $i} = $_POST['no_kursi' . $i];
    $daftar_kursi_dipesan[$i] = $_POST['no_kursi' . $i];
}

$temp = array_count_values($daftar_kursi_dipesan);
for ($i = 1; $i <= $total_tiket; $i++) {
    if (${"umur" . $i} < $data_movie["ketentuan_umur"]) {
        $status = false;
        $pesan = "One of the spectators is underage.";
    }
    if ($temp[${"no_kursi" . $i}] > 1) {
        $status = false;
        $pesan = "The reserved seat numbers cannot be the same.";
    }
    if ($data_user['saldo'] < $total_harga) {
        $status = false;
        $pesan = "Your balance is not enough";
    }
}

if ($status) {
    $query = "INSERT INTO orders VALUES('$id_pesanan', '$id_res', '$id_movie', '$total_tiket', '$tanggal', '$total_harga', 'Paid')";
    mysqli_query($koneksi, $query);
    $query2 = "UPDATE data_user SET saldo = saldo - $total_harga WHERE id = $id_res";
    mysqli_query($koneksi, $query2);

    for ($i = 1; $i <= $total_tiket; $i++) {
        $tmp_nama = ${"nama" . $i};
        $tmp_kursi = ${"no_kursi" . $i};
        $tmp_umur = ${"umur" . $i};

        $kode = "INSERT INTO seat VALUES('$tmp_kursi', '$tanggal', '$id_movie', '$id_pesanan', '$tmp_nama', '$tmp_umur')";
        mysqli_query($koneksi, $kode);
    }
}
?>
...
<head>
    ...
    <title>MovTix | Orders <?php echo $id_pesanan ?></title>
    <!-- plugins:css -->
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    <div class="row <?php if (!$status) echo "d-none" ?>">
                        <div class="col-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body d-flex justify-content-center align-items-center flex-column text-center">
                                    <img src="assets/img/checked.png" alt="" class="mb-4" width="180px">
                                    <span class="badge rounded-pill bg-primary mb-2"><?php echo $id_pesanan ?></span>
                                    <h3>Your order was successfully checked out</h3>
                                    <p>Show proof of order to cinema staff and enjoy your favorite movie!</p>
                        ...
                        <div class="col-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body d-flex justify-content-center align-items-center flex-column text-center">
                                    ...
                                    <span class="badge rounded-pill bg-primary mb-2"><?php echo $id_pesanan ?></span>
                                    <h3>Your order was not successful at checkout</h3>
                                    <p><?php echo $pesan ?></p>
                                    <form action="orders.php" method="POST">
                                        <input type="hidden" name="tanggal_pemesanan" value="<?php echo $tanggal; ?>" />
                                        <input type="hidden" name="total_tiket" value="<?php echo $total_tiket; ?>" />
                                        <input type="hidden" name="id_movie" value="<?php echo $id_movie; ?>" />
                                        <!-- other form inputs -->
                                        <button type="submit" class="btn btn-warning">Back to order form</button>
                                    </form>
                                </div>
                            </div>
                        </div>
